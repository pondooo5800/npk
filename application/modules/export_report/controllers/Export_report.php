...
class export_report extends My_Controller
{
    ...
    public function report_person($id)
    {
        ...
        $content = "<table cellspacing='0' cellpadding='0' width='100%'>
                    <tr>
                        <td width='70%'>ที่อยู่ : บ้านเลขที่ ".$data['person']['address'].' หมู่ที่ '.$data['person']['village']." ตำบล หนองป่าครั่ง อำเภอ เมือง จังหวัด เชียงใหม่</td>
        ...
        for ($i = 1; $i <= $data['count_rec']; ++$i) {
            if (!empty($data['tax'][8][$i]['notice_estimate']['year']) || !empty($data['tax'][9][$i]['notice_estimate']['year'])) {
                ...
                if ($tax_a != '' && $tax_b != '') {
                    $tax_ab = @$data['tax'][8][$i]['notice_estimate']['notice_no'].'/'.@$data['tax'][8][$i]['notice_estimate']['notice_number_p8'];
                } else {
                    $tax_ab = '';
                }
    ...
    public function usereEsimate()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            <tr>
              ...
              <td>'.number_format($value['total_interest'], 2).'</td>
              <td>'.number_format($value['sum_amount_tax'] + $value['total_interest'] - $value['tax_amount'], 2).'</td>
            </tr>';
        }
